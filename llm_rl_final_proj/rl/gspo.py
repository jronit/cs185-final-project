...
class GSPO(RLAlgorithm):
    ...
    def update(
        ...
        # finishedTODO(student): implement GSPO.
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                per_token_log_ratio = new_logp - mb.old_logprobs
                seq_log_ratio = (per_token_log_ratio * mask).sum(dim=-1).clamp(-20.0, 20.0)
                seq_ratio = torch.exp(seq_log_ratio)
                unclipped_seq = seq_ratio * adv
                clipped_seq = seq_ratio.clamp(1.0 - cfg.clip_eps, 1.0 + cfg.clip_eps) * adv
                pg_loss = -torch.min(unclipped_seq, clipped_seq).mean()
                kl = approx_kl_from_logprobs(new_logp, mb.ref_logprobs, mask)
                entropy = -masked_mean(new_logp, mask)
                with torch.no_grad():
                    is_clipped_seq = ((seq_ratio < 1.0 - cfg.clip_eps) | (seq_ratio > 1.0 + cfg.clip_eps)).float()
                    clipfrac = is_clipped_seq.mean()
