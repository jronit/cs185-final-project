...
def _rm_ppo_train_entrypoint(*args: str) -> None:
    normalized_args = _normalize_args(args, default_output_dir="runs/rm_ppo_default")
    _assert_wandb_credentials_available_if_needed(normalized_args)
    cmd = ["python", "-u", "-m", "llm_rl_final_proj.online.train_rm_ppo", *normalized_args]
    _run_subprocess_with_periodic_volume_commits(cmd)
...
def rm_ppo_train_remote(*args: str) -> None:
    _rm_ppo_train_entrypoint(*args)
